        ...
        render: function () {
            var p = this.getPosition(),
                ...
                textspan = '<span class="city-picker-span" style="' +
                    ...
                    p.height + 'px;line-height:' + (p.height - p.height/2+2) + 'px;">' +
                    (placeholder ? '<span class="placeholder">' + placeholder + '</span>' : '') +
                    '<span class="title"></span><div class="arrow"></div>' + '</span>',

                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:25px;' +
            ...
            this.$dropdown.find('.city-select')
                .each(function () {
                    ...
                    if (item) {
                         ...
                         * @User:  Leo
                         * @Date:  2016-08-12  16:13:53
                         */
                        var data_val = $('[data-toggle="city-picker"]').attr('data-val');
                        if( data_val == 'code'){ //获取单个城市编号
                            text = item.code;
                        }else if(data_val == 'code-all'){ //上级所有地区编号
                            text += ($(this).hasClass('province') ? '' : '/') + item.code;
                        }else if( data_val == 'text-all'){
                           text = item.address;  //地区名称
                        }else{
                            text += ($(this).hasClass('province') ? '' : '/') + item.address; //获取所有地区名称
                        }
                    }
                });
            return text;
