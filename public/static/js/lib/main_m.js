...
function renderKEditor(el,options) {
    ...
    In.ready('KindEditor',function() {
        var defOps = { filterMode:false,uploadPath:'Wst/KindEditor', moduleName:'', uploadJson:'',afterBlur: function(){this.sync();} }; //默认配置
        ...
        getFileUploadUrl(defOps,function(urls) { //{viewUrl:'',uploadUrl:'',uploadPathUrl:''}
...
$.postH = function(url,data,fn) {
    ...
    $.extend(data, {return_data_type:'html'} );
...
$.fn.renderFileUpload = function(options,fn) { //渲染上传按钮插件 要求图片要显示的尺寸 宽*高
    "use strict";
    var inputObjs = this;
