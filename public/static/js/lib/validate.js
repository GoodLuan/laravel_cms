...
$.fn.checkData = function(form,directShow) {
    ...
    var vFuncNames = {
        'isEmail':'Email地址',
        'isEmails':'Email地址，多个用逗号、分号、空格或换行分隔',
        'isMobilePhone':'手机号',
        'isTelephone':'电话号码、手机号或400号电话格式',
        'isMemberAccount': '手机号或邮箱格式',
        'isManagerName': '由字母、数字、空格和[.-_@&*]字符组成的字符串',
        'isUserName': '由字母、数字、空格、横线和下划线组成的字符串',
        'isChinese':'由中文、字母、数字、空格和[,.-_@*]组成的字符串',
    ...
    var vFn = {
        ...
        'isEmails' : function(v){
            ...
            var emails = v.split(',');
            for(var x in emails) { if(!vFn.isEmail(emails[x])) return false; }
            return true;
        },
        'isMobilePhone' : function(v){ return vIsEmpty || v.match(/^1[34578][0-9]{9}$/); },
        'isTelephone' : function (v) {
            if(vIsEmpty) return true;
            else if(v.match(/^(\+?86[-\s]?)?1[34578][0-9]{9}$/)) return true;
            else if(v.match(/^(\+?\d{1,5}[-\s]?)?\d{7,9}(-\d+)?$/)) return true;
            else if(v.match(/^400([-\s]\d{3,4}){2}$/)) return true;
            else if(v.match(/^(\d{2,3}[\s]){2}\d{6,9}$/)) return true;
            else return false;
        },
        'isMemberAccount':function (v) { return vFn.isMobilePhone(v) || vFn.isEmail(v);},
        'isManagerName' : function(v){ return vIsEmpty || v.match(/^\w+[\s\w-_@&\.\*]*\w+$/); },
        'isUserName' : function(v){ return vIsEmpty || v.match(/^\w+[\s\w-]*\w+$/); },
        'isChinese' : function(v){ return vIsEmpty || v.match(/^[\u4E00-\u9FA5\uFE30-\uFFA0\w-]+[\u4E00-\u9FA5\uFE30-\uFFA0\w-\s,@\.\*]*$/g); },
